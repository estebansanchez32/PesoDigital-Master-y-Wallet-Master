...
import org.PesoDigitalj.utils.Fiat;
...
public class ExchangeRatesProvider extends ContentProvider {
    ...
    private static final HttpUrl PesoDigitalAVERAGE_URL = HttpUrl
            .parse("https://apiv2.PesoDigitalaverage.com/indices/global/ticker/short?crypto=BTC");
    private static final String PesoDigitalAVERAGE_SOURCE = "PesoDigitalAverage.com";
    ...
    public Cursor query(final Uri uri, final String[] projection, final String selection, final String[] selectionArgs,
        ...
        if (selection == null) {
            for (final Map.Entry<String, ExchangeRate> entry : exchangeRates.entrySet()) {
                ...
                final org.PesoDigitalj.utils.ExchangeRate rate = exchangeRate.rate;
    ...
    public static ExchangeRate getExchangeRate(final Cursor cursor) {
        ...
        return new ExchangeRate(new org.PesoDigitalj.utils.ExchangeRate(rateCoin, rateFiat), source);
    }
    ...
    private Map<String, ExchangeRate> requestExchangeRates() {
        ...
        request.url(PesoDigitalAVERAGE_URL);
        ...
        try {
            ...
            if (response.isSuccessful()) {
                            ...
                            try {
                                ...
                                if (rate.signum() > 0)
                                    rates.put(fiatCurrencyCode, new ExchangeRate(
                                            new org.PesoDigitalj.utils.ExchangeRate(rate), PesoDigitalAVERAGE_SOURCE));
                            } catch (final IllegalArgumentException x) {
                                log.warn("problem fetching {} exchange rate from {}: {}", currencyCode,
                                        PesoDigitalAVERAGE_URL, x.getMessage());
                            }
                        }
                    }
                }
                ...
                watch.stop();
                log.info("fetched exchange rates from {}, {} chars, took {}", PesoDigitalAVERAGE_URL, content.length(),
                        watch);

                return rates;
            } else {
                log.warn("http status {} when fetching exchange rates from {}", response.code(), PesoDigitalAVERAGE_URL);
            }
        } catch (final Exception x) {
            log.warn("problem fetching exchange rates from " + PesoDigitalAVERAGE_URL, x);
        }

        return null;
    }

    // backport from PesoDigitalj 0.15
