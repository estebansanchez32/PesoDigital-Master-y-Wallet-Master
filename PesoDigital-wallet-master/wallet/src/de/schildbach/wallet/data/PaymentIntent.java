...
import org.PesoDigitalj.core.ScriptException;
...
import org.PesoDigitalj.script.Script;
import org.PesoDigitalj.script.ScriptBuilder;
...
public final class PaymentIntent implements Parcelable {
    ...
    public static PaymentIntent fromPesoDigitalUri(final PesoDigitalURI PesoDigitalUri) {
        final Address address = PesoDigitalUri.getAddress();
        final Output[] outputs = address != null ? buildSimplePayTo(PesoDigitalUri.getAmount(), address) : null;
        final String bluetoothMac = (String) PesoDigitalUri.getParameterByName(Bluetooth.MAC_URI_PARAM);
        final String paymentRequestHashStr = (String) PesoDigitalUri.getParameterByName("h");
        final byte[] paymentRequestHash = paymentRequestHashStr != null ? base64UrlDecode(paymentRequestHashStr) : null;

        return new PaymentIntent(PaymentIntent.Standard.BIP21, null, null, outputs, PesoDigitalUri.getLabel(),
                bluetoothMac != null ? "bt:" + bluetoothMac : null, null, PesoDigitalUri.getPaymentRequestUrl(),
                paymentRequestHash);
    }
