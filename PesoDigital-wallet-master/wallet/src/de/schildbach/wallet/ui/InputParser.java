...
import org.PesoDigital.protocols.payments.Protos;
import org.PesoDigitalj.core.Address;
import org.PesoDigitalj.core.AddressFormatException;
import org.PesoDigitalj.core.DumpedPrivateKey;
import org.PesoDigitalj.core.ProtocolException;
...
import org.PesoDigitalj.core.WrongNetworkException;
import org.PesoDigitalj.crypto.BIP38PrivateKey;
import org.PesoDigitalj.crypto.TrustStoreLoader;
import org.PesoDigitalj.protocols.payments.PaymentProtocol;
import org.PesoDigitalj.protocols.payments.PaymentProtocol.PkiVerificationData;
import org.PesoDigitalj.protocols.payments.PaymentProtocolException;
import org.PesoDigitalj.protocols.payments.PaymentSession;
import org.PesoDigitalj.uri.PesoDigitalURI;
import org.PesoDigitalj.uri.PesoDigitalURIParseException;
...
public abstract class InputParser {
    ...
    public abstract static class StringInputParser extends InputParser {
        ...
        @Override
        public void parse() {
            if (input.startsWith("PesoDigital:-")) {
                try {
            ...
            } else if (input.startsWith("PesoDigital:")) {
                try {
                    final PesoDigitalURI PesoDigitalUri = new PesoDigitalURI(null, input);
                    final Address address = PesoDigitalUri.getAddress();
                    if (address != null && !Constants.NETWORK_PARAMETERS.equals(address.getParameters()))
                        throw new PesoDigitalURIParseException("mismatched network");

                    handlePaymentIntent(PaymentIntent.fromPesoDigitalUri(PesoDigitalUri));
                } catch (final PesoDigitalURIParseException x) {
                    log.info("got invalid PesoDigital uri: '" + input + "'", x);

                    error(R.string.input_parser_invalid_PesoDigital_uri, input);
                }
