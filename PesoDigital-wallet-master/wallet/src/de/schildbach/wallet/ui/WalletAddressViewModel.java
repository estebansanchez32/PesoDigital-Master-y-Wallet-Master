...
import org.PesoDigitalj.wallet.listeners.WalletChangeEventListener;
...
import org.PesoDigitalj.wallet.listeners.WalletReorganizeEventListener;
...
public class WalletAddressViewModel extends AndroidViewModel {
    ...
    public final MediatorLiveData<Uri> PesoDigitalUri = new MediatorLiveData<>();
    ...
    public WalletAddressViewModel(final Application application) {
        ...
        this.PesoDigitalUri.addSource(currentAddress, new Observer<Address>() {
            @Override
            public void onChanged(final Address currentAddress) {
                maybeGeneratePesoDigitalUri();
            }
        });
        this.PesoDigitalUri.addSource(ownName, new Observer<String>() {
            @Override
            public void onChanged(final String label) {
                maybeGeneratePesoDigitalUri();
            }
    ...
    private void maybeGeneratePesoDigitalUri() {
        final Address address = currentAddress.getValue();
        if (address != null) {
            PesoDigitalUri.setValue(Uri.parse(uri(address, ownName.getValue())));
        }
    }

    private String uri(final Address address, final String label) {
        return PesoDigitalURI.convertToPesoDigitalURI(address, null, label, null);
    }
