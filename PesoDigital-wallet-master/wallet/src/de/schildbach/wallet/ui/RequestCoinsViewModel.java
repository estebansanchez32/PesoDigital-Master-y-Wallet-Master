...
public class RequestCoinsViewModel extends AndroidViewModel {
    ...
    public RequestCoinsViewModel(final Application application) {
        ...
        this.PesoDigitalUri.addSource(freshReceiveAddress, new Observer<Address>() {
            @Override
            public void onChanged(final Address receiveAddress) {
        ...
        this.PesoDigitalUri.addSource(amount, new Observer<Coin>() {
            @Override
            public void onChanged(final Coin amount) {
                maybeGeneratePesoDigitalUri();
            }
    ...
    private void maybeGeneratePesoDigitalUri() {
        final Address address = freshReceiveAddress.getValue();
        if (address != null) {
            PesoDigitalUri.setValue(Uri.parse(uri(address, amount.getValue(), ownName.getValue(), null)));
        }
    }

    private String uri(final Address address, final Coin amount, final String label, final String bluetoothMac) {
        final StringBuilder uri = new StringBuilder(PesoDigitalURI.convertToPesoDigitalURI(address, amount, label, null));
