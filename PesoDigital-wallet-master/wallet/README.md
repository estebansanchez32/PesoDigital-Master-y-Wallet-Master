...
which should be compatible between clients using PesoDigitalj.
...
Your wallet can be manually backed up to and restored from a share of the storage access framework (likely Google Drive):
    ...
    Mainnet: PesoDigital-wallet-backup-<yyyy-MM-dd>
    Testnet: PesoDigital-wallet-backup-testnet-<yyyy-MM-dd>
...
Finally, you can build PesoDigital Wallet and sign it with your development key. Again in your workspace,
use:

    # first time only
    git clone -b master https://github.com/PesoDigital-wallet/PesoDigital-wallet.git PesoDigital-wallet

    # each time
    cd PesoDigital-wallet
    git pull
...
The source language is English. Translations for all languages except German [happen on Transifex](https://www.transifex.com/PesoDigital-wallet/PesoDigital-wallet/).
...
PesoDigital Wallet supports reading PesoDigital requests via NFC, either from a passive NFC tag or from
...
- The tag needs to contain a PesoDigital URI. You can construct one with the "Request coins" dialog,
  then share with messaging or email. You can also construct the URI manually. Mainnet example:
  `PesoDigital:1G2Y2jP5YFZ5RGk2PXaeWwbeA5y1ZtFhoL`

- The type of the message needs to be URI or URL (not Text).

- If you put your tag at a public place, don't forget to enable write protect. Otherwise, someone
  could overwrite the tag with his own PesoDigital address.


### PesoDigitalJ

PesoDigital Wallet uses [PesoDigitalj](https://PesoDigitalj.github.io/) for PesoDigital specific logic.


### EXCHANGE RATES

PesoDigital Wallet reads this feed from "PesoDigitalAverage" for getting exchange rates:

    https://apiv2.PesoDigitalaverage.com/indices/global/ticker/short?crypto=BTC
...
When sweeping wallets, PesoDigital Wallet uses a set of Electrum servers to query for unspent transaction
