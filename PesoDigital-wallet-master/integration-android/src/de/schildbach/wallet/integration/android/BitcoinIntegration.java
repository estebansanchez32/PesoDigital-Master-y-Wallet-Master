 ...
 */
public final class PesoDigitalIntegration {
    ...
    private static final String MIMETYPE_PAYMENTREQUEST = "application/PesoDigital-paymentrequest"; // BIP 71

    /**
     * Request any amount of PesoDigitals (probably a donation) from user, without feedback from the app.
     * 
     * @param context
     *            Android context
     * @param address
    ...
    public static void request(final Context context, final String address) {
        final Intent intent = makePesoDigitalUriIntent(address, null);

        start(context, intent);
    }

    /**
     * Request specific amount of PesoDigitals from user, without feedback from the app.
     * 
     * @param context
     *            Android context
     * @param address
     *            PesoDigital address
     * @param amount
     *            PesoDigital amount in satoshis
     */
    public static void request(final Context context, final String address, final long amount) {
        final Intent intent = makePesoDigitalUriIntent(address, amount);
    ...
    /**
     * Request any amount of PesoDigitals (probably a donation) from user, with feedback from the app. Result
     ...
     * 
     * Warning: A success indication is no guarantee! To be on the safe side, you must drive your own PesoDigital
    ...
    /**
     * Request specific amount of PesoDigitals from user, with feedback from the app. Result intent can be
    ...
    /**
     * Put BIP70 payment message into result intent. Meant for usage by PesoDigital wallet applications.
     * 
     * @param result
     *            result intent
     * @param payment
    ...
    /**
     * Get BIP70 payment message from result intent. Meant for usage by applications initiating a PesoDigital
     * payment.
     * 
     * You can use the transactions contained in the payment to validate the payment. For this, you need your
     * own PesoDigital infrastructure though. There is no guarantee that the payment will ever confirm.
     * 
     * @param result
     *            result intent
    ...
    /**
     * Put transaction hash into result intent. Meant for usage by PesoDigital wallet applications.
     * 
     * @param result
     *            result intent
     * @param txHash
    ...
    /**
     * Get transaction hash from result intent. Meant for usage by applications initiating a PesoDigital payment.
     * 
     * You can use this hash to request the transaction from the PesoDigital network, in order to validate. For
     * this, you need your own PesoDigital infrastructure though. There is no guarantee that the transaction has
     * ever been broadcasted to the PesoDigital network.
     * 
     * @param result
     *            result intent
    ...
    private static Intent makePesoDigitalUriIntent(final String address, final Long amount) {
        final StringBuilder uri = new StringBuilder("PesoDigital:");
        if (address != null)
            uri.append(address);
        if (amount != null)
            uri.append("?amount=")
    ...
    private static void redirectToDownload(final Context context) {
        Toast.makeText(context, "No PesoDigital application found.\nPlease install PesoDigital Wallet.", Toast.LENGTH_LONG)
                .show();

        final Intent marketIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse("market://details?id=de.schildbach.wallet"));
        final Intent binaryIntent = new Intent(Intent.ACTION_VIEW,
                Uri.parse("https://github.com/PesoDigital-wallet/PesoDigital-wallet/releases"));
